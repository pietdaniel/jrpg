...
Graphics::Graphics(int winWidth, int winHeight, const char* winTitle) {
	...
	TTF_Init();
	m_screen = SDL_SetVideoMode(winWidth, winHeight, 0, SDL_HWSURFACE | SDL_DOUBLEBUF); // SDL_FULLSCREEN, 
...
SDL_Surface* Graphics::loadBitmap(const char* bitmapFileName, int transR, int transG, int transB) {
	...
	SDL_SetColorKey(bitmap, SDL_SRCCOLORKEY, SDL_MapRGB(bitmap->format, transR, transG, transB)); // SDL_RLEACCEL - compresses, keys, uncompresses
	return bitmap;	
}
